...
export default function WearShareApp() {
  ...
  return (
    <>
      ...
      {page==='explore' && <ExplorePage user={user} toast={toast} refreshCart={refreshCart} onLogin={()=>setShowLogin(true)}/>}
...
function HomePage({ user, setPage, onRegister }) {
  ...
  const features = [
    {icon:'fa-tag',t:'Affordable Luxury',d:'Premium brands at a fraction of retail'},
    {icon:'fa-shield-halved',t:'Secured Payments',d:'Razorpay-powered with deposit protection'},
    {icon:'fa-leaf',t:'Eco-Friendly',d:'Reduce fashion waste, rent don\'t buy'},
    {icon:'fa-people-group',t:'Community First',d:'Built for sharing within your community'},
    {icon:'fa-wand-magic-sparkles',t:'AI Try-On',d:'See how clothes look on you with AI'},
    {icon:'fa-truck-fast',t:'Easy Pickup',d:'Simple community-based collection & return'},
  ]
  return (
    <div style={{background:D}}>
      ...
      <section style={{maxWidth:1100,margin:'0 auto',padding:'2rem 1.5rem 3rem'}}>
        <h2 style={{textAlign:'center',fontSize:'2rem',fontWeight:900,marginBottom:'.5rem'}} className="gt">Why WearShare?</h2>
        <p style={{textAlign:'center',color:'#64748B',marginBottom:'2.5rem'}}>The smartest way to access premium fashion</p>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:'1.25rem'}}>
          {features.map(f=>(
            <div key={f.icon} className="card" style={{padding:'1.75rem',textAlign:'center'}}>
              <i className={`fas ${f.icon}`} style={{fontSize:'2.2rem',marginBottom:'.75rem',display:'block',color:P2}}/>
              <h3 style={{fontWeight:700,marginBottom:'.4rem'}}>{f.t}</h3>
              <p style={{color:'#64748B',fontSize:'.88rem',lineHeight:1.5}}>{f.d}</p>
            </div>
          ))}
        </div>
      </section>
...
function CartModal({ show, onClose, toast, user, refreshCart, setPage }) {
  ...
  const checkout = async () => {
    ...
    try {
      ...
      const openRzp = (data) => {
        const rzp = new window.Razorpay({ key: data.keyId, amount: data.amount, currency: data.currency, name: 'WearShare', description: 'Clothing Rental', order_id: data.orderId, theme: { color: '#7C3AED' },
